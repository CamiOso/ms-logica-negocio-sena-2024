...
export class AdministradorDeArchivosController {
  ...
  async CargarArchivoMina(
    @inject(RestBindings.Http.RESPONSE) response: Response,
    @requestBody.file() request: Request,
  ): Promise<object | false> {
    const filePath = path.join(__dirname, ConfiguracionGeneral.carpetaArchivosMinas);
    let res = await this.StoreFileToPath(
      filePath,
      ConfiguracionGeneral.campoDeMina,
      request,
      response,
  ...
  async CargarArchivoMuestra(
    @inject(RestBindings.Http.RESPONSE) response: Response,
    @requestBody.file() request: Request,
  ): Promise<object | false> {
    const filePath = path.join(__dirname, ConfiguracionGeneral.carpetaArchivosMuestras);
    let res = await this.StoreFileToPath(
      filePath,
      ConfiguracionGeneral.campoDeMuestra,
      request,
      response,
      ConfiguracionGeneral.extensionesImagenes,
    );
  ...
  async CargarArchivoPozo(
    @inject(RestBindings.Http.RESPONSE) response: Response,
    @requestBody.file() request: Request,
  ): Promise<object | false> {
    const filePath = path.join(__dirname, ConfiguracionGeneral.carpetaArchivosPozos);
    let res = await this.StoreFileToPath(
      filePath,
      ConfiguracionGeneral.campoDePozo,
      request,
      response,
      ConfiguracionGeneral.extensionesImagenes,
    );
  ...
  private ObtenerArchivosPorTipo(tipo: number) {
    ...
    switch (tipo) {
      // amusement
      case 1:
        filePath = path.join(__dirname, ConfiguracionGeneral.carpetaArchivosMinas);
        break;
      case 2:
        filePath = path.join(__dirname, ConfiguracionGeneral.carpetaArchivosMuestras);
        break;
      case 3:
